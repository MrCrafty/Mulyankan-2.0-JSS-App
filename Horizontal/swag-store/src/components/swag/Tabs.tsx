                    ...
                    <Tab.List className="-mb-px flex space-x-8">
                        {fields?.Items.map((item, index) => (
                            <Tab className={({selected}) => clsx(
                                ...
                                'border-transparent text-gray-700 hover:border-gray-300 hover:text-gray-800',
                                'whitespace-nowrap border-b-2 py-6 text-sm font-medium')}>{item.fields.Title.value}</Tab>
                        ))}
                    </Tab.List>
                </div>
                        ...
                        <Tab.Panel>
                                ...
                                {item.fields.Items.map((itm, idx) => {
                                    ...
                                    <div key={idx} className="flex space-x-4 text-sm text-gray-500">
                                        <div className="flex-none py-10">
                                            <img src={itm.fields.Image.value.src} alt="" className="h-10 w-10 rounded-full bg-gray-100" />
                                        </div>
                                        <div className={clsx(idx === 0 ? '' : 'border-t border-gray-200', 'py-10')}>
                                            <h3 className="font-medium text-gray-900">{itm.fields.Title.value}</h3>
                                            <p>
                                                <time dateTime={itm.fields.Subtitle.value}>{itm.fields.Subtitle.value}</time>
                                            </p>
                                            <div
                            ...
                            {item.fields.Type?.value === 'faq' && <>
                                ...
                                {item.fields.Items.map((itm, idx) => {
                                    return (
                                        <Fragment key={idx}>
                                            <dt className="mt-10 font-medium text-gray-900">{itm.fields.Title.value}</dt>
                                            <dd className="prose prose-sm mt-2 max-w-none text-gray-500">
                                            <p dangerouslySetInnerHTML={{ __html: itm.fields.Description.value }}></p>
                                            </dd>
                                        </Fragment>
                                    )
                                })}
